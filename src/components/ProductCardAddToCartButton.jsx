...
export default function ProductCardAddToCartButton({ product }) {
  const { addToCart, cart, updateQuantity, isInitialized } = useCart();
  const itemId = product?.slug || product?._id || product?.id || product?.productId || product?.Name || product?.name;
  const cartItem = cart.find((item) => item.id === itemId);
  const quantity = cartItem?.quantity || 0;
  const showQuantityControls = isInitialized && quantity > 0;
  ...
  return (
    <div
      className={cn(
        ...
        showQuantityControls ? 'w-[5.5rem]' : 'w-8'
      )}
      data-state={showQuantityControls ? 'quantity' : 'idle'}
    >
